...
tf.app.flags.DEFINE_string('data_dir', FILEPATH + '/cifar10_data',  # please change this to current data path!!
...
def _variable_on_cpu(name, shape, initializer):
    with tf.device('/cpu:0'):
        type = tf.float16 if FLAGS.use_fp16 else tf.float32
...
def _variable_with_weight_decay(name, shape, stddev, wd):
    type = tf.float16 if FLAGS.use_fp16 else tf.float32
    variable = _variable_on_cpu(name, shape, tf.truncated_normal_initializer(stddev=stddev, dtype=type))
    if wd is not None:
...
def inputs(eval_data):
    ...
    images, labels = read_data.inputs(eval_data=eval_data, data_dir=data_dir, batch_size=FLAGS.batch_size)
